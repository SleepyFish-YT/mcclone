...
void GlStateManager::blendFunc_(int src, int dst) {
    if (src != blendState.srcFactor || dst != blendState.dstFactor || src != blendState.srcFactorAlpha ||
        dst != blendState.dstFactorAlpha) {
...
void GlStateManager::tryBlendFuncSeparate_(int src, int dst, int srcA, int dstA) {
    if (src != blendState.srcFactor || dst != blendState.dstFactor || srcA != blendState.srcFactorAlpha ||
        dstA != blendState.dstFactorAlpha) {
        blendState.srcFactor = src;
        blendState.dstFactor = dst;
...
void GlStateManager::bindCurrentTexture_() {
...
void GlStateManager::clearColor_(float r, float g, float b, float a) {
    if (r != clearState.color.red || g != clearState.color.green || b != clearState.color.blue ||
        a != clearState.color.alpha) {
...
void GlStateManager::glTexCoordPointer_(int size, int type, int stride, int offset) {
    ::glTexCoordPointer(size, type, stride, (void *) (intptr_t) offset);
}

void GlStateManager::glTexCoordPointer_(int size, int type, int stride, const void *ptr) {
    ::glTexCoordPointer(size, type, stride, ptr);
}

void GlStateManager::glVertexPointer_(int size, int type, int stride, int offset) {
    ::glVertexPointer(size, type, stride, (void *) (intptr_t) offset);
}

void GlStateManager::glVertexPointer_(int size, int type, int stride, const void *ptr) {
    ::glVertexPointer(size, type, stride, ptr);
}

void GlStateManager::glColorPointer_(int size, int type, int stride, int offset) {
    ::glColorPointer(size, type, stride, (void *) (intptr_t) offset);
}

void GlStateManager::glColorPointer_(int size, int type, int stride, const void *ptr) {
...
void
GlStateManager::glTexImage2D_(int target, int level, int internalFormat, int width, int height, int border, int format,
                              int type, const int *pixels) {
    ::glTexImage2D(target, level, internalFormat, width, height, border, format, type, pixels);
}

void
GlStateManager::glTexSubImage2D_(int target, int level, int xoffset, int yoffset, int width, int height, int format,
                                 int type, const int *pixels) {
    ::glTexSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels);
}

void GlStateManager::glCopyTexSubImage2D_(int target, int level, int xoffset, int yoffset, int x, int y, int width,
                                          int height) {
