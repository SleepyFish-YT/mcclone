...
public:
    ...
    static std::string_view getUsageDisplayName(EnumUsage usage) {
        switch (usage) {
            case EnumUsage::POSITION:
                return "Position";
            case EnumUsage::NORMAL:
                return "Normal";
            case EnumUsage::COLOR:
                return "Vertex Color";
            case EnumUsage::UV:
                return "UV";
            case EnumUsage::MATRIX:
                return "Bone Matrix";
            case EnumUsage::BLEND_WEIGHT:
                return "Blend Weight";
            case EnumUsage::PADDING:
                return "Padding";
            default:
                return "Unknown";
        }
...
public:
    ...
    std::string toString() const {
        return std::to_string(this->elementCount) + "," + std::string(getUsageDisplayName(this->usage)) + "," +
               std::string(getTypeInfo(this->type).displayName);
    }

    bool operator==(const VertexFormatElement &other) const {
        return this->elementCount == other.elementCount && this->index == other.index && this->type == other.type &&
