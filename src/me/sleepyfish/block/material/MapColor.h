...
public:
    ...
    static constexpr int ALPHA_MAP[] = {180, 220, 255, 135}; // this might be wrong: {135, 180, 220, 255}
    ...
    int getMapColor(const int lightLevel) const {
        const int alpha = (lightLevel < 4) ? MapColor::ALPHA_MAP[lightLevel] : 0;
        if (alpha == 0) return 0;
    ...
    MapColor(const int index, const int color) : colorIndex(index), colorValue(color) {
        if (index < 0 || index > 63) {
